...
func (dt *GenericDataTable) editableRowName(gtx layout.Context, th *material.Theme, rowIndex int) layout.Dimensions {
	// 直接從 datatable 取得最新 rowName
	...
	// 若不在編輯模式，呈現可點擊文字模式
	if dt.editingRowName != rowIndex {
		// 若不在編輯模式，確保 editor 內容與資料同步
		if dt.editingRowName != rowIndex {
			if editor.Text() != rowName {
				editor.SetText(rowName)
			}
			return clicker.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
				// 使用與一般標題儲存格相同的背景處理
	...
	return layout.Stack{}.Layout(gtx,
		layout.Stacked(func(gtx layout.Context) layout.Dimensions {
			...
			innerRect := clip.RRect{
				Rect: image.Rectangle{
					Min: image.Pt(2, 2),
					Max: image.Pt(cellWidth-2, cellHeight-2),
				},
				NE: 4,
				SE: 4,
				SW: 4,
